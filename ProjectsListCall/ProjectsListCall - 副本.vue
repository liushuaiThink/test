        ...
        <div class="con-l">
            <div style="padding:5px 5px 10px 5px">
              ...
              <el-row style="margin:10px 0;text-align:center;color:#99A9BF">
                ...
                <span style="width:20%;float:left">{{quality.long}}</span>
              </el-row>
            ...
            <el-form :model="ruleForm" :rules="rules" :label-position="labelPosition" label-width="100px" ref="ruleForm" class="demo-ruleForm" style="margin:0 0 0 0" v-if="record.is_qc==2?true:false">
              ...
              <el-form-item label="合格级别：" prop="hege" v-if="ruleForm.result==1?true:false">
            ...
            <el-form :model="ruleForm" :rules="rules" :label-position="labelPosition" label-width="100px" ref="ruleForm" class="demo-ruleForm" style="margin:0 0 0 0" v-else>
        ...
        <div style="height:108px;overflow-y:auto;padding:10px" class="answer">
          <p v-for="(item,index) in testPaper" :key="index" style="text-align:center;line-height:30px;margin-bottom:10px" class="paper">
            <el-popover trigger="hover" placement="top" >
              <p style="width:200px">姓名: {{item.q_title}}</p>
              <div slot="reference" class="name-wrapper">
                <el-tag >
                  <span style="width:10%;float:left">Q{{index+1}}</span>
                  <span>{{item.answer.alp?item.answer.alp:'未选择'}}{{item.answer.answered?'：'+item.answer.answered:''}}</span>
                </el-tag>
              </div>
            </el-popover>
          </p>
  ...
  data() {
    return {
      ...
      rules:{
        ...
        type: [
          { required: true, type: 'array',message: '请输入活动名称', trigger: 'blur' },
        ],
        comment: [
          { required: true, message: '请输入活动名称', trigger: 'blur' },
        ],
  ...
  methods: {
        ...
        if (res.data.code==1000&&res.data.data) {
            ...
            if (res.data.data.record) {
              ...
              if (this.num==1) {
                  ...
                  wavesurfer.load('../../../static/lixiang.mp3');
                ...
                wavesurfer.load('../../../static/ggh.mp3');
              }
            }    
    ...
    exportToExcel(isAll) {
      ...
      if (this.searchedTableData.length>0) {
        ...
        console.log(data)
        this.$message('正在导出');
        window.location.href='http://192.168.2.200:8080/yunying/statistics/export?admin_token='+localStorage.token+'&project_id='+Number(this.$route.query.id)+'&type='+this.form.call+'&call_status='+this.form.call_result+'&call_result_type='+this.form.jiesuanstandard[0]+'&settlement_result='+this.form.jiesuanstandard[1]+'&quality='+this.form.quality+'&start_long='+this.form.start_long+'&end_long='+this.form.end_long+'&phone='+this.form.phone
        // var name = '数据表',
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          let data={
            ...
            standard_id:this.quality.call_status,
...
.el-tag{background: #fff;color:#000;font-size: 14px;width: 100%}
